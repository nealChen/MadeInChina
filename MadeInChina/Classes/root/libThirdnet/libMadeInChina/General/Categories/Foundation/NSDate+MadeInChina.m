...
@implementation NSDate (MadeInChina)
...
- (NSString *)briefTimeInWords {
	...
	if (seconds < 60.0) {
		if (seconds < 2.0) {
			return MadeInChinaLocalizedString(@"1s");
		}
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%ds"), (NSInteger)seconds];
	}
	...
	if (minutes >= 0.0 && minutes < 60.0) {
		if (minutes < 2.0) {
			return MadeInChinaLocalizedString(@"1m");
		}
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%dm"), (NSInteger)minutes];
	}
	...
	// Hours
		...
		if (hours < 2) {
			return MadeInChinaLocalizedString(@"1h");
		}
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%dh"), hours];
	}
	...
	// Days
		...
		if (days < 2) {
			return MadeInChinaLocalizedString(@"1d");
		}
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%@d"),
	...
	// Years
		...
		if (years < 2) {
			return MadeInChinaLocalizedString(@"1y");
		}
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%@y"),
...
+ (NSString *)timeInWordsFromTimeInterval:(NSTimeInterval)intervalInSeconds includingSeconds:(BOOL)includeSeconds {
	...
	if (intervalInMinutes >= 0 && intervalInMinutes <= 1) {
		if (!includeSeconds) {
			return intervalInMinutes <= 0 ? MadeInChinaLocalizedString(@"less than a minute") : MadeInChinaLocalizedString(@"1 minute");
		}
		if (intervalInSeconds >= 0 && intervalInSeconds < 5) {
			return [NSString stringWithFormat:MadeInChinaLocalizedString(@"less than %d seconds"), 5];
		} else if (intervalInSeconds >= 5 && intervalInSeconds < 10) {
			return [NSString stringWithFormat:MadeInChinaLocalizedString(@"less than %d seconds"), 10];
			...
			return MadeInChinaLocalizedString(@"half a minute");
		} else if (intervalInSeconds >= 40 && intervalInSeconds < 60) {
			return MadeInChinaLocalizedString(@"less than a minute");
	 	} else {
			return MadeInChinaLocalizedString(@"1 minute");
		}
	} else if (intervalInMinutes >= 2 && intervalInMinutes <= 44) {
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%d minutes"), (NSInteger)intervalInMinutes];
	} else if (intervalInMinutes >= 45 && intervalInMinutes <= 89) {
		return MadeInChinaLocalizedString(@"about 1 hour");
	} else if (intervalInMinutes >= 90 && intervalInMinutes <= 1439) {
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"about %d hours"), (NSInteger)round(intervalInMinutes / 60.0f)];
	} else if (intervalInMinutes >= 1440 && intervalInMinutes <= 2879) {
		return MadeInChinaLocalizedString(@"1 day");
	} else if (intervalInMinutes >= 2880 && intervalInMinutes <= 43199) {
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%d days"), (NSInteger)round(intervalInMinutes / 1440.0f)];
	} else if (intervalInMinutes >= 43200 && intervalInMinutes <= 86399) {
		return MadeInChinaLocalizedString(@"about 1 month");
	} else if (intervalInMinutes >= 86400 && intervalInMinutes <= 525599) {
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"%d months"), (NSInteger)round(intervalInMinutes / 43200.0f)];
	} else if (intervalInMinutes >= 525600 && intervalInMinutes <= 1051199) {
		return MadeInChinaLocalizedString(@"about 1 year");
	} else {
		return [NSString stringWithFormat:MadeInChinaLocalizedString(@"over %d years"), (NSInteger)round(intervalInMinutes / 525600.0f)];
	}
	return nil;
}
